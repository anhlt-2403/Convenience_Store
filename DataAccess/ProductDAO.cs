    ...
    public class ProductDAO
    {
        ...
        public List<TblProduct> GetAllProduct()
        {
            ...
            try
            {
                ...
                {
                    listProduct = ctx.TblProducts.Include(m => m.Category).Include(m => m.Status).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        public List<TblProduct> GetAllAvailableProduct()
        {
            ...
            try
            {
                using (var ctx = new prn211group4Context())
                {
                    listProduct = ctx.TblProducts.Include(m => m.Status).Include(n => n.Category).Where(product => product.Quantity == 0).ToList();
                }
            }
            catch (Exception ex)
            {
